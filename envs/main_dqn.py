...
def main(args):
    ...
    env = ServerlessEnv(env_config={"render_mode":None, "size":num_service})
    # print(len(env.action_mask))
    ...
    state, _ = env.reset()
    # state = spaces.flatten(env.state_space,state)
    ...
    for e in range(episodes):
        ...
        while not done:
            ...
            # next_state = spaces.flatten(env.state_space,next_state)
            next_state = np.reshape(next_state, [state_size])
            agent.store_transition(state, action, reward, next_state, done)
            state = next_state
            agent.learn()
            rewards.append(reward)
            print("Action:{}.....\nNext_state:{}".format(action,next_state))
            cum_reward += reward
        ...
        print("episode: {}/{}, sub_episode: {}, reward: {}".format(e, episodes, env.current_time, cum_reward))
        if e > 100:
